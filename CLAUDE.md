...
6. **household API를 수정할 때 Express(`server/routes/households.js`)와 Vercel 진입점(`api/households/current.js`, `api/households/index.js`, `api/household-api.js` + rewrites)을 모두 검증해야 하며, localhost 성공만으로 완료 처리하지 않는다.** non-Next Vercel에서는 `[...route]` catch-all과 `api/` 아래 helper 파일을 쓰지 않는다(함수 개수·배포 실패 원인). 라우팅·응답은 `server/lib/household-api-handler.js` 공통 dispatcher를 유지하고 `node scripts/test-household-api-contract.mjs`를 통과시킨다. 배포 후 production에서 `X-Household-Handler` / `X-Deploy-Commit` 헤더로 새 함수 실행을 확인한다.
...
- `api/households/current.js`, `api/households/index.js`, `api/household-api.js`
- `vercel.json` (household rewrites)
