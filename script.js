...
function updateHomeRecipesSubtitle() {
  ...
  const count = RecommendationService.countMakeableNow(RecipeRepository.getHomeRecipes());
  if (count > 0) {
    el.textContent = `🥕 현재 재료로 바로 만들 수 있는 레시피 ${count}개`;
  } else {
    el.textContent = '🥕 현재 재료로 만들 수 있는 레시피가 없습니다. 부족한 재료를 추가해보세요.';
  }
  el.hidden = false;
}
...
/** 홈 카드 준비 상태 문구 — 긍정형, 재료명 우선 */
function formatHomeReadyMessage(missing) {
  const names = (missing || []).map(shortIngredientLabel).filter(Boolean);
  const count = names.length;
  if (count <= 0) {
    return { label: '바로 가능', mod: 'available' };
  }
  if (count === 1) {
    return { label: `${names[0]}만 있으면 가능`, mod: 'low' };
  }
  if (count === 2) {
    return { label: `${names[0]}, ${names[1]}만 더 준비하면 가능`, mod: 'low' };
  }
  if (count <= 4) {
    return { label: `${names[0]}, ${names[1]} 외 ${count - 2}개 더 필요해요`, mod: 'medium' };
  }
  return { label: '장보면 바로 만들 수 있어요', mod: 'high' };
}
...
function formatHomeSubstitutionLine(substituted) {
  ...
  return {
    required,
    owned,
    html: `${esc(required)}는 <span class="recipe-card-home__sub-hl">${esc(owned)}</span>로 대체 가능합니다.`,
  };
}
...
function homeRecipeCardHTML(result) {
  ...
  const statusRow = missingCount > 0
    ? `<div class="recipe-card-home__row recipe-card-home__row--status">
        <span class="recipe-card-status recipe-card-status--${status.mod}">${esc(status.label)}</span>
    ...
    <div class="recipe-card recipe-card--home" role="button" tabindex="0" data-rid="${esc(recipe.id)}">
      ...
      <div class="recipe-card__body">
        <div class="recipe-card-home__row recipe-card-home__row--title">
          ...
          <div class="recipe-card-home__title-end">${saveBtn}${matchPill}</div>
        </div>
...
function renderHomeFilters() {
  ...
  dom.homeFilterChips.innerHTML = FILTERS.map((f) => `
  ...
  dom.homeFilterChips.querySelectorAll('[data-home-f]').forEach((chip) => {
    chip.onclick = () => {
      ...
      } else if (!dom.menuSearchInput?.value.trim()) {
        toggleHomeFilterPanel(false);
...
function renderHome() {
  ...
  if (names.length && dom.homeTodayHero) {
    const naengtulResults = RecommendationService.recommendNaengtul(homeRecipes);
    dom.homeTodayHero.hidden = false;
    dom.homeTodayHero.innerHTML = homeNaengtulHTML(naengtulResults);
...
function init() {
  ...
  dom.homeRecipesSeeAll?.addEventListener('click', () => {
    dom.recipeList?.closest('.section--home-recipes')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  });
